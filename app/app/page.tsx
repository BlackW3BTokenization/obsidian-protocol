...
export default function PitchPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 overflow-x-hidden">
      ...
      <section className="pt-10 pb-16 md:pt-16 md:pb-20">
        <div className="grid grid-cols-1 gap-10 md:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] md:items-center">
          <div className="relative">
            ...
            >
              Tokenized precious metals on Solana. Mint, send, or burn to redeem physical
              metal. Backed 1:1 by AGX vaults. 400ms settlement.
            </p>
            ...
            <div className="flex flex-wrap gap-3 mt-6 relative">
              {[
                ...
                { label: "AGX Reserve (UPMA)", href: "https://upma.org" },
              ].map(({ label, href }) => (
                <a
                  key={label}
                  href={href}
          ...
          {/* Hero asset card · xGOLD — Change 3: SVG replaces photo */}
          <div className="relative">
            ...
            >
              ...
              {/* ── Change 3: geometric SVG placeholder ── */}
              <TokenSvg symbol={xgold.symbol} className="absolute inset-0" />

              {/* Token badge */}
              <span
                className="absolute font-display font-black tracking-[0.2em] uppercase"
                style={{
                  top: 16, left: 16, fontSize: 10,
                  padding: "4px 12px",
              ...
              <div
                className="absolute"
                style={{ bottom: 16, left: 16, right: 16, display: "flex", justifyContent: "space-between", alignItems: "flex-end", zIndex: 2 }}
              >
                <div>
                  <p className="font-display font-bold uppercase mb-1" style={{ fontSize: 8, letterSpacing: "0.3em", color: "var(--gray)" }}>
                    Spot price / oz
                  </p>
                  <p className="font-display font-black tabular-nums" style={{ fontSize: 28, color: "var(--gold-light)", letterSpacing: "-0.01em", lineHeight: 1 }}>
                    ${liveGoldPrice >= 1000
                ...
                <div className="text-right">
                  ...
                  <p className="font-display font-black tabular-nums" style={{ fontSize: 18, color: liveChange24h.startsWith("+") ? "var(--mint-green)" : "var(--burn-red)", lineHeight: 1 }}>
                    {liveChange24h}
                  </p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-3 gap-px mt-px" style={{ background: "var(--carbon)" }}>
              {[
                ...
                <div key={s.label} className="px-3 py-3" style={{ background: "var(--void)" }}>
                  <p className="font-display font-black tabular-nums" style={{ fontSize: 16, color: "var(--gold-light)", letterSpacing: "-0.01em", lineHeight: 1 }}>
                    {s.value}
                  </p>
                  <p className="font-display font-bold uppercase mt-1.5" style={{ fontSize: 7, letterSpacing: "0.25em", color: "var(--gray)" }}>
                    {s.label}
                  </p>
                </div>
              ))}
      ...
      {/* ── Change 4: AGX credibility block — first thing after hero ── */}
      <div className="text-center py-5 border-y" style={{ borderColor: "var(--carbon)" }}>
        <p
          className="font-display font-black uppercase tracking-[0.3em]"
          style={{ fontSize: 10, color: "var(--parchment)" }}
        >
          Vault-custodied by Alpine Gold Exchange&nbsp;·&nbsp;60,000+ insured accounts&nbsp;·&nbsp;Signed institutional partnership.
        </p>
      </div>
      ...
      {/* Trailing CTA — Change 1: sub copy trimmed */}
      <section className="my-16 grid grid-cols-1 sm:grid-cols-3 gap-3">
        {([
          { icon: "laptop_trading"  as FintechIconName, title: "Mint live",   sub: "Connect a wallet to mint xGOLD",              href: "/protocol",   cta: "Open dApp" },
          { icon: "safe"            as FintechIconName, title: "Verify",      sub: "ZK-attested AGX reserves · 5 tokens",         href: "/reserves",   cta: "View reserves" },
          { icon: "dollar_contract" as FintechIconName, title: "Build on it", sub: "x402-gated APIs · pay per call · &lt;400ms",  href: "/developers", cta: "Read API spec" },
          ...
          <Link
            ...
            style={{ background: "var(--void)", borderColor: "var(--carbon)", outlineColor: "var(--vault-gold)" }}
          >
            <div aria-hidden="true" className="absolute -right-4 -top-4 opacity-25 group-hover:opacity-40 transition-opacity">
              <FintechIcon name={icon} size={96} />
            </div>
            <FintechIcon name={icon} size={36} glow className="mb-3" />
            <p className="font-display text-[10px] font-black tracking-[0.3em] mb-2 relative" style={{ color: "var(--gold)" }}>
              {title.toUpperCase()}
            </p>
            <p className="text-sm relative" style={{ color: "var(--parchment)" }} dangerouslySetInnerHTML={{ __html: sub }} />
            <p className="font-display text-[10px] font-black tracking-[0.25em] mt-4 inline-flex items-center gap-1.5 relative" style={{ color: "var(--gold-light)" }}>
